...
if (!$company_id || empty($driver)) die("Invalid Parameters");
...
$query = "
    SELECT 
        id, date, invoice_number, location, kg, 
        total_freight_cost_usd, total_freight_cost_iqd, 
        paid_to_driver_usd, paid_to_driver_iqd, '' as note
    FROM purchases
    WHERE $where
...
foreach ($transactions as $t) {
    $total_kg += (float)$t['kg'];
    $total_cost_usd += (float)$t['total_freight_cost_usd'];
    $total_cost_iqd += (float)$t['total_freight_cost_iqd'];
...
<head>
    <meta charset="UTF-8">
    <title>کەشف حیسابی شۆفێر - <?= htmlspecialchars($driver) ?></title>
    <style>
        @font-face {
            font-family: 'Rabar21';
...
<div class="print-page">
    ...
    <div class="meta-grid">
        ...
        <div class="meta-item"><span class="meta-label">ناوی شۆفێر:</span><span class="meta-value"><?= htmlspecialchars($driver) ?></span></div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-danger"><?= (float)$t['total_freight_cost_usd'] > 0 ? number_format($t['total_freight_cost_usd'], 2) : '-' ?></td>
                <td class="text-danger"><?= (float)$t['total_freight_cost_iqd'] > 0 ? number_format($t['total_freight_cost_iqd']) : '-' ?></td>
                <td class="text-success"><?= (float)$t['paid_to_driver_usd'] > 0 ? number_format($t['paid_to_driver_usd'], 2) : '-' ?></td>
                <td class="text-success"><?= (float)$t['paid_to_driver_iqd'] > 0 ? number_format($t['paid_to_driver_iqd']) : '-' ?></td>
