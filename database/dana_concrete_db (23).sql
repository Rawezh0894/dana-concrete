...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 09, 2025 at 04:24 PM
...
(1, 'چاوی ١', 'چاو', 'لمی ڕەش', 73530.00, 1650.60, 0.02),
(2, 'چاوی ٢', 'چاو', 'لمی کەسارە', 33170.00, 0.00, 0.00),
(3, 'چاوی ٣', 'چاو', 'چەو', 99970.00, 864.90, 0.01),
(4, 'چاوی ٤', 'چاو', 'چەو', 14085.00, 0.00, 0.00),
(5, 'سایلۆی ١', 'سایلۆ', 'چیمەنتۆ', 43465.00, 0.00, 0.00),
(6, 'سایلۆی ٢', 'سایلۆ', 'چیمەنتۆ', 33145.00, 0.00, 0.00),
(7, 'تەنکی دەرمان ١', 'تەنکی', 'دەرمان', 49814.50, 0.00, 0.00),
...
(1, 'ماس', 700.05, 1417000.00, 0.00, 0.00, 'دینار'),
(12, 'ڕاوێژ', 984.00, 0.00, 500.00, 0.00, 'دینار'),
...
(4, 'حاجی', '07709240895', '', 0.00, 507.50, 0.00, 0.00),
...
CREATE TRIGGER `trg_after_insert_customer_debt_payments` AFTER INSERT ON `customer_debt_payments` FOR EACH ROW BEGIN
  IF NEW.paid_iqd > 0 OR NEW.paid_usd > 0 THEN
    ...
    VALUES (
      ...
      NEW.paid_iqd,
      NEW.paid_usd,
      CASE WHEN NEW.paid_usd > 0 THEN 'دۆلار' ELSE 'دینار' END,
      CONCAT('گەڕاندنەوەی قەرز: کڕیار ', NEW.customer_id),
      NEW.customer_id
    );
  END IF;
END
...
CREATE TRIGGER `trg_after_insert_debt_payments` AFTER INSERT ON `debt_payments` FOR EACH ROW BEGIN
  IF NEW.amount_iqd > 0 OR NEW.amount_usd > 0 THEN
    INSERT INTO cash_box (date, type, amount_iqd, amount_usd, currency, note, created_by)
    VALUES (
      NEW.date,
      'deposit',
      NEW.amount_iqd,
      NEW.amount_usd,
      CASE WHEN NEW.amount_usd > 0 THEN 'دۆلار' ELSE 'دینار' END,
      CONCAT('گەڕاندنەوەی قەرز: کۆمپانیا ', NEW.company_id),
      NEW.created_by
    );
  END IF;
END
...
CREATE TRIGGER `trg_after_insert_employee_payments` AFTER INSERT ON `employee_payments` FOR EACH ROW BEGIN
  IF NEW.total > 0 THEN
    ...
    VALUES (
      ...
      NEW.total,
      0,
      'دینار', -- یان 'دۆلار' ئەگەر بە دۆلار پارە دەدرێت
      CONCAT('پارەدان بە کارمەند: ', NEW.employee_id),
      NEW.employee_id
    );
  END IF;
END
...
CREATE TRIGGER `trg_after_insert_other_expenses` AFTER INSERT ON `other_expenses` FOR EACH ROW BEGIN
  IF NEW.paid_iqd > 0 OR NEW.paid_usd > 0 THEN
    ...
    VALUES (
      ...
      NEW.paid_usd,
      NEW.currency_type,
      CONCAT('خەرجی تر: invoice ', NEW.invoice_number),
      NEW.employee_id
    );
  END IF;
END
...
CREATE TRIGGER `trg_after_insert_person_other_expenses_debt_payments` AFTER INSERT ON `person_other_expenses_debt_payments` FOR EACH ROW BEGIN
  IF NEW.amount_iqd > 0 OR NEW.amount_usd > 0 THEN
    ...
    VALUES (
      ...
      NEW.amount_usd,
      CASE WHEN NEW.amount_usd > 0 THEN 'دۆلار' ELSE 'دینار' END,
      CONCAT('گەڕاندنەوەی قەرز: کەس ', NEW.person_id),
      NEW.person_id
    );
  END IF;
END
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_after_insert_purchase` AFTER INSERT ON `purchases` FOR EACH ROW BEGIN
  INSERT INTO cash_box (date, type, amount_iqd, amount_usd, currency, note, created_by)
  VALUES (
    NEW.date,
    'withdraw',
    ...
    NEW.paid_usd,
    NEW.type,
    CONCAT('کڕین: invoice ', NEW.invoice_number),
    NEW.company_id
  );
END
$$
...
(9, 70, '2025-07-02', '55', 'ئەیوب', 'سلێمانی', 1, 36000.00, 0.00, 'قەرز', 150000, 1, 'دینار', 0, 0, 0.00, 1088000, 1, 1188000.00, 33000.00, 0.00, '2025-07-08 16:16:20');
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_after_insert_sale_for_cash_box` AFTER INSERT ON `sales` FOR EACH ROW BEGIN
  IF NEW.amount_paid_usd > 0 OR NEW.amount_paid_iq > 0 THEN
    INSERT INTO cash_box (date, type, amount_iqd, amount_usd, currency, note, created_by)
    VALUES (
      NEW.order_date,
      'deposit',
      NEW.amount_paid_iq,
      NEW.amount_paid_usd,
      'دۆلار', -- یان 'دینار' بەپێی پارەی داوە
      CONCAT('فرۆشتن: invoice ', NEW.invoice_number),
      NEW.customer_id
    );
  END IF;
END
