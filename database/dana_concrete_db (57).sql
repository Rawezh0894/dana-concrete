...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 04, 2025 at 09:11 PM
...
(1, 'چاوی ١', 'چاو', 'لمی کەسارە', 0.00, 0.00, 0.0000000000),
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_before_delete_sale` BEFORE DELETE ON `sales` FOR EACH ROW BEGIN
    DECLARE v_black_sand_kg DECIMAL(10,2);
    DECLARE v_brown_sand_kg DECIMAL(10,2);
    DECLARE v_gravel_bin3_kg DECIMAL(10,2);
    DECLARE v_gravel_bin4_kg DECIMAL(10,2);
    ...
    -- گەڕاندنەوەی ماتریاڵەکان بۆ bins_silos (بە kg)
    IF v_black_sand_kg > 0 THEN
        UPDATE bins_silos 
        SET amount = amount + v_black_sand_kg, 
            total_value = (amount * average_price)
