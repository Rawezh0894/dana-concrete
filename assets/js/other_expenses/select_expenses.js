async function loadOtherExpenses() {
    try {
        ...
        // Cache globally for delegated handlers
        window.otherExpensesData = data;
        console.log('Cached other expenses data globally', { count: Array.isArray(data) ? data.length : 0 });
        ...
        try {
            ...
            if (document.getElementById('edit_expense_type')) {
                ...
                if (row.expense_type === 'بەکارهێنانی گاز') {
                    setTimeout(() => { populateGasPurchasePrice('edit'); }, 100);
                }
            }
            if (document.getElementById('edit_material_id')) {
                ...
                setTimeout(() => {
                    if (row.material_id) { populateMaterialPrices(row.material_id, 'edit'); }
                }, 100);
            }
