...
class ReceiptManager {
    ...
    renderSalesData(response) {
            ...
            try {
                ...
                return `
                    <tr class="receipt-row" data-receipt-id="${row.invoice_number || ''}">
                        ...
                        <td class="col-ratio">${row.rezh || ''}</td>
                        <td class="col-formula-type">${row.formula_type || ''}</td>
                        <td class="col-unit-price">${this.formatCurrency(row.price_per_unit)}</td>
                        <td class="col-total">${this.formatCurrency(row.total_price)}</td>
                        <td class="col-remaining">${this.formatCurrency(row.remaining_amount)}</td>
                        <td class="col-invoice">${this.formatReceiptNumber(row.invoice_number)}</td>
                        <td class="col-date">${this.formatDate(row.order_date)}</td>
                    </tr>
                `;
            } catch (error) {
        ...
        this.lastTotalQuantity = response.total_quantity;
    ...
    updateSummary(total, remainingTotal, totalQuantity = null) {
        ...
        const currentTotalQuantity = totalQuantity || this.lastTotalQuantity || '0.00 م³';
            ...
            tfoot.innerHTML = `
                <tr class="summary-row">
                    <td colspan="${totalColspan}" style="text-align: center;">
                        ...
                        کۆی پێوانە: ${currentTotalQuantity}
                    </td>
                </tr>
            `;
        }
    ...
    updateDebtSummary(openingDebt, remainingTotal) {
        ...
        if (currentSalesRemaining !== undefined && currentSalesRemaining !== null) {
            ...
            salesRemainingValue = typeof this.lastGrossTotal === 'number' ? this.lastGrossTotal : 0;
        }
        ...
        const netTotal = Math.max(0, Math.round((salesRemainingValue - totalDiscount) * 100) / 100);
        const totalDebtReduction = Math.round((totalPaid + totalDiscount) * 100) / 100;
        const calculatedRemaining = Math.max(0, Math.round((salesRemainingValue - totalDebtReduction) * 100) / 100);
