...
function loadDriversSummary() {
    if (driversSummaryTable) {
        driversSummaryTable.destroy();
    }

    driversSummaryTable = $('#driversSummaryTable').DataTable({
        ajax: {
            url: '../process/company_profile/select_drivers_summary.php',
            type: 'GET',
            data: function (d) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    return `<button onclick="openDriverLedger('${row.driver}')" class="btn btn-sm btn-teal text-white rounded-pill px-3"><i class="fas fa-eye me-1"></i> بینینی کەشف حیساب</button>`;
                }
            }
        ],
        language: {
            url: "https://cdn.datatables.net/plug-ins/1.13.6/i18n/ku.json",
            emptyTable: "هیچ داتایەک نەدۆزرایەوە بۆ شۆفێرەکان"
        },
        order: [[0, 'asc']]
    });
}
...
function openDriverLedger(driverName) {
    ...
    if (driverLedgerTable) {
        driverLedgerTable.destroy();
    }

    driverLedgerTable = $('#driverLedgerTable').DataTable({
        ajax: {
            url: '../process/company_profile/select_drivers_ledger.php',
            type: 'GET',
            data: {
        ...
        columns: [
            ...
            { 
                ...
                title: 'پارەی دراو',
                render: function(data, type, row) {
                    let usd = parseFloat(row.paid_to_driver_usd);
                    let iqd = parseFloat(row.paid_to_driver_iqd);
