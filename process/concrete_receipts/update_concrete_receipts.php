...
try {
    $stmt = $pdo->prepare('UPDATE concrete_receipts SET receipt_number=?, customer_id=?, location=?, meter_amount=?, formulas_id=?, pump_car_id=?, pump_driver_id=?, mixer_car_id=?, mixer_driver_id=? WHERE id=?');
    $stmt->execute([
        $receipt_number,
        $customer_id ?: null,
        $location,
