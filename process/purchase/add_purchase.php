...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO purchases (date, invoice_number, driver, location, material_id, amount_iqd, kg, price, payment_type, exchange_rate, company_id, type, paid_usd, paid_iqd, remaining_usd, remaining_iqd, bin_id, price_per_kg_iqd, price_per_kg_usd, freight_price_per_kg_iqd, freight_price_per_kg_usd, total_freight_cost_iqd, total_freight_cost_usd, paid_to_location_iqd, paid_to_location_usd, paid_to_driver_iqd, paid_to_driver_usd, factory_truck_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $ok = $stmt->execute([
            $date, $invoice_number, $driver, $location, $material_id, $amount_iqd, $kg, $price, $payment_type, $exchange_rate, $company_id, $type, $paid_usd, $paid_iqd, $remaining_usd, $remaining_iqd, $bin_id, $price_per_kg_iqd, $price_per_kg_usd, $freight_price_per_kg_iqd, $freight_price_per_kg_usd, $total_freight_cost_iqd, $total_freight_cost_usd, $paid_to_location_iqd, $paid_to_location_usd, $paid_to_driver_iqd, $paid_to_driver_usd, $factory_truck_id
        ]);
        if ($ok) {
            // Update company debt
