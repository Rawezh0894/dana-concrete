...
try {
    // Check if receipt number already exists (excluding current record)
    if (!empty($current_id)) {
        $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM purchase_materials WHERE receipt_number = ? AND id != ?");
        $stmt->execute([$receipt_number, $current_id]);
    } else {
