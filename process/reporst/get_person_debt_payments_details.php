...
try {
    ...
    if ($from_date && $to_date) {
        $date_condition = ' AND pedp.date >= ? AND pedp.date <= ?';
        $params[] = $from_date;
        $params[] = $to_date;
    } elseif ($from_date) {
        $date_condition = ' AND pedp.date >= ?';
        $params[] = $from_date;
    } elseif ($to_date) {
        $date_condition = ' AND pedp.date <= ?';
        $params[] = $to_date;
    ...
    // Get all debt payments with person names
    $query = "
        SELECT 
            pedp.id,
            pedp.person_id,
            ...
            pedp.amount_usd,
            pedp.amount_iqd,
            COALESCE(pedp.discount_usd, 0) as discount_usd,
            COALESCE(pedp.discount_iqd, 0) as discount_iqd,
            pedp.note,
            p.name AS person_name
        FROM person_other_expenses_debt_payments pedp
        LEFT JOIN other_expense_persons p ON pedp.person_id = p.id
        WHERE 1=1 $date_condition
        ORDER BY pedp.date DESC, pedp.id DESC
    ";
    ...
    error_log('PDOException in get_person_debt_payments_details.php: ' . $e->getMessage());
    error_log('Stack trace: ' . $e->getTraceAsString());
    echo json_encode(['success' => false, 'error' => 'هەڵە لە وەرگرتنی وردەکاریەکان: ' . $e->getMessage()]);
} catch (Exception $e) {
    error_log('Exception in get_person_debt_payments_details.php: ' . $e->getMessage());
    error_log('Stack trace: ' . $e->getTraceAsString());
    echo json_encode(['success' => false, 'error' => 'هەڵە لە وەرگرتنی وردەکاریەکان: ' . $e->getMessage()]);
}
