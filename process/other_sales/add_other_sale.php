...
try {
    ...
    if (!$customer_id || !$item_type || empty($item_name) || $quantity <= 0) {
    ...
    if (!empty($invoice_number)) {
        $dup_stmt = $pdo->prepare("SELECT COUNT(*) FROM other_sales WHERE invoice_number = ?");
    ...
    $item_type_name = $item_type_names[$item_type] ?? $item_type;

    // Insert other sale
    $sql = "INSERT INTO other_sales (customer_id, recipient, location, item_type, item_name, quantity, unit, price_per_unit, total_price, payment_type, amount_paid_usd, amount_paid_iq, remaining_amount, dolar_rate, discount, order_date, invoice_number, notes) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        $customer_id, $recipient, $location, $item_type, $item_name, $quantity, $unit, $price_per_unit, $total_price,
    ...
    $additional_info = [
        'action_type' => 'other_sale_creation',
    ...
    createDetailedNotification(
        ...
        'other_sales',
        $sale_id,
        "فرۆشتی دیکە زیادکرا (invoice: $invoice_number, کڕیار: {$customer['name']}, کاڵا: $item_name, بڕ: $quantity $unit)",
        null,
        $new_values,
        $additional_info,
        getUserIP()
    ...
    echo json_encode([
        'success' => true,
        'message' => 'فرۆشتی دیکە بە سەرکەوتوویی زیادکرا',
        'sale_id' => $sale_id
    ]);

} catch (Exception $e) {
    error_log("Error in add_other_sale.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
